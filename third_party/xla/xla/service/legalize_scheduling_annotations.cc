...
bool HasUserWithSameAnnotation(
    HloInstruction* instr,
    absl::flat_hash_map<HloInstruction*, int64_t>& annotation) {
  CHECK(annotation.contains(instr));
  return absl::c_any_of(instr->users(), [&annotation,
                                         &instr](HloInstruction* user) {
    return annotation.contains(user) && annotation[user] == annotation[instr];
  });
}
...
absl::StatusOr<bool> LegalizeSchedulingAnnotations::Run(
    ...
    for (HloInstruction* instr : computation->instructions()) {
      ...
      if (!absl::SimpleAtoi(attrs.at("_scheduling_group_id"), &annotation_id)) {
        return absl::InvalidArgumentError(absl::StrCat(
            "Instruction has an invalid scheduling annotation, inst: ",
  ...
  for (const auto& [id, annotated_instructions] : annotation_to_instructions) {
    // First find the annotation roots.
    std::vector<HloInstruction*> stack;
    absl::flat_hash_set<HloInstruction*> visited;
    for (HloInstruction* instr : annotated_instructions) {
      CHECK(annotation.contains(instr));
      CHECK_EQ(annotation[instr], id);
      if (HasUserWithSameAnnotation(instr, annotation)) {
        continue;
      }
      stack.push_back(instr);
      visited.insert(instr);
      VLOG(2) << "Annotation group: " << id << " root: " << instr->name();
    }
    VLOG(1) << "Annotation group: " << id << " has " << stack.size()
            << " roots";
    // Traverse the HLO graph starting from the annotation roots and moving to
    // the users. If there are gaps in the annotation, the traversal will hit
